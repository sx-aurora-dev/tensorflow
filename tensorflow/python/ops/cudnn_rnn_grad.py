...
@ops.RegisterGradient("VERNN")
def _ve_rnn_backward(op, *grads):
  """Gradients for the VERNN op."""
  if not op.get_attr("is_training"):
    raise ValueError(
        "To use VERNN in gradients, is_training must be set to True.")
  return gen_cudnn_rnn_ops.vernn_backprop(
      input=op.inputs[0],
      input_h=op.inputs[1],
      input_c=op.inputs[2],
      kernel=op.inputs[3],
      recurrent_kernel=op.inputs[4],
      bias=op.inputs[5],
      output=op.outputs[0],
      output_h=op.outputs[1],
      output_c=op.outputs[2],
