...
REGISTER_OP("ZeroOut")
    ...
    .Input("to_zero: NumTensors * T")
    .Output("zeroed: NumTensors * T")
 ...
 public:
  explicit ZeroOutOp(OpKernelConstruction* context) : OpKernel(context) {
    OP_REQUIRES_OK(context,
                   context->GetAttr("NumTensors", &NumTensors_));

    OP_REQUIRES(context, NumTensors_ >= 0,
                errors::InvalidArgument("Need NumTensors >= 0, got ",
                                        NumTensors_));
  ...
  void Compute(OpKernelContext* context) override {
    ...
    for( int i=0; i<NumTensors_ ; i++ ) {
      ...
      OP_REQUIRES_OK(context, context->allocate_output(i, input_tensor.shape(),
						       &output_tensor));
	...
	output_flat(j) = T(0);
      ...
      if (N > preserve_index_) output_flat(preserve_index_) = input(preserve_index_);
    }
  }
...
#define REGISTER_KENREL(type)				\
REGISTER_KERNEL_BUILDER(Name("ZeroOut")			\
                          .Device(DEVICE_CPU)		\
			  .TypeConstraint<type>("T"),	\
			ZeroOutOp<type>)

REGISTER_KENREL(int32) ;
REGISTER_KENREL(float) ;
#undef REGSITER_KERNEL
...
VE_REGISTER_KENREL(int32) ;
VE_REGISTER_KENREL(float) ;
#undef VE_REGISTER_KENREL
#endif
