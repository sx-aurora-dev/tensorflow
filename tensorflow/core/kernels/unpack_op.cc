...
template <typename T>
class UnpackOp<VEDevice, T> : public VEOpKernel {
 public:
  explicit UnpackOp(OpKernelConstruction* context) : VEOpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("axis", &axis_));
  }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    Args.addArg<int64>(axis_dim) ;
    Args.addArg<int64>(after_dim) ;
