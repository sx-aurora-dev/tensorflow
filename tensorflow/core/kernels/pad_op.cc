...
#if 0 // debug
    VLOG(1) << "====== input_shape ======        : " << input_shape;
    VLOG(1) << "====== paddings_as_tensor ====== : ";
    VLOG(1) << "                    shape        - " << paddings_as_tensor.shape();
    VLOG(1) << "                    dims         - " << paddings_as_tensor.dims();
    VLOG(1) << "                    dtype        - " << DataTypeString(paddings_as_tensor.dtype());
#endif
...
#if 0 // debug
      VLOG(1) << " ====== paddings ====== : " << paddings(i,0) << " " << paddings(i,1);
#endif
...
#if 0 // debug
    VLOG(1) << " [[[OperateWithVariableRank]]] ";
    VLOG(1) << "    ------ fixed_dims         : " << fixed_dims;
    VLOG(1) << "    ------ input.shape        : " << input.shape();
    VLOG(1) << "    ------       dims         : " << input.dims();
    VLOG(1) << "    ------       dtype        : " << input.dtype();
    VLOG(1) << "    ------ paddings           : " << paddings;
    VLOG(1) << "    ------ pad_value          : " << pad_value;
    VLOG(1) << "    ------ output.shape       : " << output->shape();
    VLOG(1) << "    ------        dims        : " << output->dims();
    VLOG(1) << "    ------        dtype       : " << output->dtype();
#endif
...
class VEPadOp : public OpKernel {
 public:
  explicit VEPadOp(OpKernelConstruction* context) : OpKernel(context) {}
  void Compute(OpKernelContext* context) override {
    const Tensor& in0 = context->input(0);
    const Tensor& in1 = context->input(1);
    const int dims = in0.dims();
...
#if 0 // debug
    VLOG(1) << "  in0            = " << std::hex << &in0
	      << std::dec <<  "(" << in0.DebugString(4) << ")";
    VLOG(1) << "  in1            = " << std::hex << &in1
	      << std::dec <<  "(" << in1.DebugString(4) << ")";
#endif
    ...
    if (dims > 1 && CollapseAdjacentNonPaddedDimensions(
      ...
      VLOG(1) << "** add args **";
#endif
      int32_t cidims = collapsed_input_shape.dims();

      VEOpKernelHelper::ArgsImpl<> args;
      args.addArg<int32_t>(cidims);           // 0 : 
      args.addArg<int64>(DataTypeToEnum<T>::value);
                                              // 1 : type of Tensor 
      args.addArg<Tensor>(collapsed_input);   // 2 : Tensor before padding
      args.addArg<int64>(DataTypeToEnum<Tpadding>::value);
                                              // 3 : type of pad_value 
      args.addArg<T>(pad_value);              // 4 : pad value
      args.addArg<Tensor>(collapsed_output);  // 5 : Tensor after padding
      for (int i = 0; i< cidims; i++) {       // 6 : paddings
	args.addArg<int32_t>(collapsed_paddings_matrix(i, 0));
	args.addArg<int32_t>(collapsed_paddings_matrix(i, 1));
      }

#if 0 // debug
      VLOG(1) << "** call kernel **";
#endif
      VEOpKernelHelper::Call(context, "Pad", args);
...
#define REGISTER_VE_KERNEL(T)                                  \
  ...
  //TF_CALL_GPU_ALL_TYPES(REGISTER_VE_KERNEL);
  //TF_CALL_int8(REGISTER_VE_KERNEL);
  //TF_CALL_uint8(REGISTER_VE_KERNEL);
  TF_CALL_float(REGISTER_VE_KERNEL);
  TF_CALL_double(REGISTER_VE_KERNEL);

#undef REGISTER_VE_KERNEL

// A special VE kernel for int32.
REGISTER_KERNEL_BUILDER(Name("Pad")
                            .Device(DEVICE_VE)
                            .TypeConstraint<int32>("T")
                            .TypeConstraint<int32>("Tpaddings")
