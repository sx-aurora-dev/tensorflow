...
template <typename T>
class BroadcastToOp<VEDevice, T> : public OpKernel {
 public:
  explicit BroadcastToOp(OpKernelConstruction* ctx) : OpKernel(ctx) {}
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    // Handle broadcast from Scalar.
    if (input_shape.dims() == 0) {
      functor::VEFillFunctor<T>(ctx, output_tensor, input_tensor);
      return;
    }
    ...
    functor::VEBroadcastTo<T>()(ctx, *output_tensor, output_shape,
                                input_tensor, input_shape, bcast);
  }
};
