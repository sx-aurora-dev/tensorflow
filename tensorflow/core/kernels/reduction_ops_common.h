...
class VEReductionOp : public OpKernel {
 public:
  explicit VEReductionOp(OpKernelConstruction* ctx, std::string name)
...
#define DEFINE_VE_REDUCTION_OP(Name) \
template <typename T, typename Tperm> \
class VE##Name##Op : public VEReductionOp<T,Tperm> { \
  public: \
    explicit VE##Name##Op(OpKernelConstruction* ctx)  \
      : VEReductionOp<T,Tperm>(ctx, #Name) {} \
};

#define REGISTER_VE_REDUCTION_OP(NAME, T)                            \
